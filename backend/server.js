...
// CORS – env-based allowlist (Render/Netlify)
app.use(
  cors({
    origin: (origin, cb) => {
...
// ---- Hospital → file mapping
// 1 -> Eastbourne, 2 -> Conquest, 3 -> Royal Sussex (rsch), 4 -> QVH
...
// extra CORS for local dev (can stay, but first one already covers us)
  ...
  if (json && typeof json === "object") {
    ...
    for (const v of Object.values(json)) {
      if (Array.isArray(v)) out.push(...v);
        ...
        for (const iv of Object.values(v))
          if (Array.isArray(iv)) out.push(...iv);
      }
    }
    return out;
  }
...
function safeLoad(filePath) {
  try {
    ...
    try {
      ...
      return {
        ok: false,
        error: `JSON parse error in ${path.basename(filePath)}: ${e.message}`,
      };
    }
  } catch (e) {
    return {
      ok: false,
      error: `File read error for ${path.basename(filePath)}: ${e.message}`,
    };
  }
}

// ⚠️ THIS WAS DROPPING `building` – now we add it.
function toUniform(items) {
  return items.map((it) => ({
    ...
    // 👇 NEW: keep building from QVH and any other dataset
    building: it.building || it.Building || "",
...
(function verifyFiles() {
  ...
  for (const [id, file] of Object.entries(HOSPITAL_FILES)) {
    ...
    console.log(
      `  Hospital ${id} -> ${file} : ${exists ? "✅ found" : "❌ MISSING"}`
    );
  }
})();
...
app.get("/api/hospitals/:id", (req, res) => {
  ...
  if (!fs.existsSync(fp))
    return res
      .status(500)
      .json({ message: "Data file not found", filePath: fp });
...
app.get("/api/search", (req, res) => {
  ...
  if (!q || !hospital)
    return res
      .status(400)
      .json({ message: "Missing query param `q` or `hospital`" });
  ...
  // 👇 also search in `building`, so "building 17" etc. works
  const fields = [
    "name",
    "floor",
    "areaColor",
  ...
  res.json(matches); // empty array is still 200
});
